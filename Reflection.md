...
This prototype connects directly to our HW2 architecture because it demonstrates a client-server design, HTTP communication, backend file handling, and database persistence.
...
The main prototype files include:

- `server.js`: Express backend server and API routes
- `database.js`: SQLite database setup
- `wildspot-ui/app/(tabs)/index.tsx`: React Native / Expo UI prototype
- `uploads/`: local folder where uploaded image files are stored
- `wildspot.db`: local SQLite database file generated by the prototype

These files work together to show the basic communication path between the UI, backend server, local file storage, and database.
...
After a successful upload, the `/api/posts` response should include a new post with a `media_url` value similar to:

```text
/uploads/xxxxx-animal.jpg
```
...
This also helped me understand the difference between storing data and storing files. The database stores structured information about the post, while the server file system stores the actual uploaded image.
...
Another small challenge was making sure the frontend was editing the correct Expo file. The project used Expo Router, so the main UI file was located at `wildspot-ui/app/(tabs)/index.tsx` instead of only using `App.js`.

---

## Current Limitations

This prototype is not a full production version of Wildspot. It does not include real authentication, real GPS location, complete moderation features, or real Firebase Storage. It is mainly meant to prove the upload communication flow.

The current uploaded images are stored locally in the server’s `uploads/` folder. This works for a prototype, but it would not be enough for a real deployed app because the files would only exist on one local machine.

The Map, Profile, and Chat screens are also mostly UI prototypes at this stage. The Upload screen is the main working flow that communicates with the backend.
...
- connecting comments and saved posts to the backend
- adding basic validation for file type and file size
...
The prototype also helped me better understand how client-server communication works in a real application. The frontend does not store the post by itself. Instead, it sends the data to the backend, and the backend controls how the data and image files are saved.
